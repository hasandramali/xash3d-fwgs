...
bool CLocalizedStringTable::AddFile( IFileSystem *fileSystem, const char *fileName )
{
	...
	if( pszLanguage )
	{
		...
		{
			Q_strncpy( szFileName, fileName, uiLangStart );
	...
	auto data = reinterpret_cast<uchar16*>( malloc( size + 2 ) );

	fileSystem->Read( data, size, hFile );

	data[ size / sizeof( uchar16 ) ] = '\0';
	...
	if( *data == 0xFEFF )
	{
		...
		char key[ 128 ];
		uchar16 keytoken[ 128 ];
		uchar16 valuetoken[ 2048 ];
...
void CLocalizedStringTable::ReloadLocalizationFiles( IFileSystem *filesystem )
{
	...
	{
		AddFile( filesystem, m_LocalizationFiles[ i ].String() );
	}
}
