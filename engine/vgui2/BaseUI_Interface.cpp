...
void CBaseUI::Initialize( CreateInterfaceFn* factories, int count )
{
	...
	// VGUI2 is statically linked into the engine - use this module's factory
	m_FactoryList[ 1 ] = Sys_GetFactoryThis();

	// chromehtml not available
	m_FactoryList[ 3 ] = nullptr;
...
void CBaseUI::Start( cl_enginefunc_t* engineFuncs, int interfaceVersion )
{
	...
	if( m_FactoryList[ 3 ] )
	{
		pHTMLController = reinterpret_cast<IHTMLChromeController*>( m_FactoryList[ 3 ]( CHROMEHTML_CONTROLLER_INTERFACE_VERSION, nullptr ) );
	}
