...
- (IBAction)OnResourcesReady:(id)sender
{
	...
	UIView *lastPage = self.TutorContent.arrangedSubviews.lastObject;
	if (lastPage && ![lastPage viewWithTag:99]) {
		UIButton *fmBtn = [UIButton buttonWithType:UIButtonTypeSystem];
		[fmBtn setTitle:@"File Manager" forState:UIControlStateNormal];
		fmBtn.tag = 99;
		fmBtn.translatesAutoresizingMaskIntoConstraints = NO;
		[fmBtn addTarget:self action:@selector(openFileManager) forControlEvents:UIControlEventTouchUpInside];
		[lastPage addSubview:fmBtn];
		[NSLayoutConstraint activateConstraints:@[
			[fmBtn.centerXAnchor constraintEqualToAnchor:lastPage.centerXAnchor],
			[fmBtn.bottomAnchor constraintEqualToAnchor:lastPage.bottomAnchor constant:-8],
		]];
	}
}
