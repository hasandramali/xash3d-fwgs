...
if [ -d "$BUILDDIR" ]; then
    ...
    if [ -d "$ASSETS_DIR" ] && command -v xcrun &>/dev/null; then
        ...
        xcrun actool "$ASSETS_DIR" --compile ios/xash3d.app --app-icon AppIcon --platform iphoneos --minimum-deployment-target 12.0 --output-partial-info-plist ios/xash3d.app/ActoolInfo.plist 2>&1 || echo "Warning: asset catalog compilation failed, icon may be missing"
